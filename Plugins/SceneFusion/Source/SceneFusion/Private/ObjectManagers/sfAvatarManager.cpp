...
#define LOG_CHANNEL "sfAvatarManager"
...
sfAvatarManager::sfAvatarManager() :
    m_leftId{ -1 },
    m_rightId{ -1 },
...
void sfAvatarManager::Initialize()
{
    ...
    m_onCameraMovedHandle = FEditorDelegates::OnEditorCameraMoved.AddRaw(this, &sfAvatarManager::OnCameraMoved);
    ...
    m_cameraObjPtr = sfObject::Create(sfType::Avatar, cameraPropertiesPtr);
    if (m_isInXRMode)
    {
...
sfDictionaryProperty::SPtr sfAvatarManager::CreateAvatarProperty(
    int meshId,
...
void sfAvatarManager::CleanUp()
{
    ...
    m_sessionPtr->Delete(m_cameraObjPtr);
...
void sfAvatarManager::RegisterPropertyChangeHandlers()
{
...
void sfAvatarManager::OnCreate(sfObject::SPtr objPtr, int childIndex)
{
...
void sfAvatarManager::OnDelete(sfObject::SPtr objPtr)
{
    AsfAvatarActor* actorPtr = m_sfObjToActor.FindRef(objPtr->Id());
    if (IsActorValid(actorPtr))
    {
...
void sfAvatarManager::OnPropertyChange(sfProperty::SPtr propertyPtr)
{
    //update camera actor
...
void sfAvatarManager::OnUserLeave(sfUser::SPtr& userPtr)
{
...
void sfAvatarManager::OnUserColorChange(sfUser::SPtr userPtr)
{
...
void sfAvatarManager::OnCameraMoved(const FVector& location,
...
void sfAvatarManager::LoadMaterialAndMeshes()
{
...
uint32_t sfAvatarManager::GetOwnerId(sfObject::SPtr objPtr)
{
    while (objPtr->Parent())
    {
...
bool sfAvatarManager::InXRMode()
{
    return GEngine->XRSystem.IsValid() && IVREditorModule::Get().GetVRMode() != nullptr;
}

void sfAvatarManager::HideUserAvatar()
{
...
void sfAvatarManager::SendChange()
{
...
bool sfAvatarManager::GetCameraLocationAndRotation(FVector& location, FQuat& rotation)
{
    if (InXRMode())
    {
...
void sfAvatarManager::GetXRDeviceLocationAndRotation(int deviceId, FVector& location, FQuat& rotation)
{
...
void sfAvatarManager::SendControllerTransformToServer()
{
    if (m_isInXRMode)
    {
...
void sfAvatarManager::SendTransform(
...
void sfAvatarManager::ToggleFlashlightOnController(AsfAvatarActor* controllerActorPtr, bool flashlightOn)
{
...
bool sfAvatarManager::IsActorValid(AsfAvatarActor* actorPtr)
{
    return actorPtr && actorPtr->IsValidLowLevel() && !actorPtr->IsPendingKill();
}

void sfAvatarManager::MoveViewportToUser(uint32_t userId)
{
...
uint32_t sfAvatarManager::Follow(uint32_t userId)
{
    AsfAvatarActor* cameraActorPtr = m_userIdToCamera.FindRef(userId);
...
void sfAvatarManager::StartFollowing()
{
...
void sfAvatarManager::MoveViewportTowardsFollowedCamera()
{
...
void sfAvatarManager::SetAvatarVisibility(bool showAvatar)
{
    m_showAvatar = showAvatar;
}

void sfAvatarManager::RecreateAllAvatars()
{
    for (auto iter = m_sfObjToActor.CreateConstIterator(); iter; ++iter)
    {
